...
cp ~/repos/youtube-recipe-pipeline/SKILL.md ~/.claude/commands/youtube-recipe.md
```
...
`SKILL.md` を `~/.claude/commands/youtube-recipe.md` に配置すると、Claude Code のどのセッションからでも `/youtube-recipe` コマンドでレシピ変換を実行できる。`_transcripts/` 内の文字起こしファイルを読み取り、対話的にレシピ化する。
